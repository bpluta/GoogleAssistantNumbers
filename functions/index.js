...
const appService = builder.buildService();
...
app.intent('finish_push_setup', (conv, params) => {
  if (conv.arguments.get('PERMISSION')) {
    ...
    return appService.setNotification(db, userID, () => {
...
app.intent('set_number',(conv,params) => {
  ...
  return appService.setNumber(db, newNumber, userID, (liczba) => {
...
app.intent('set_random_number',(conv,params) => {
  const newNumber = appService.updateX()
  const userID = conv.user.id

  return appService.setNumber(db, newNumber.toString(), userID, (liczba) => {
    notify(newNumber,conv)
...
app.intent('get_number',(conv,params) => {
  const userID = conv.user.id;
  return appService.getNumber(db, userID,(liczba) => {
...
app.intent('guess_number',(conv,params) => {
  ...
  return appService.guessNumber(db,userID,type,number,lang,
